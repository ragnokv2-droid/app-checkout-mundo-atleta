  ...
  function LeadCard({
    ...
    return (
      <View style={styles.card}>
        <Text style={styles.cardTitle}>{lead.nome || "Sem nome"}</Text>
        ...
        <View style={styles.row}>
          {!!lead.telefone && (
            <TouchableOpacity style={styles.chip} onPress={() => wa(lead)}>
  ...
  return (
    <View style={styles.root}>
      <View style={styles.header}>
        <View>
          <Text style={styles.headerTitle}>Mundo Atleta</Text>
      ...
      >
        {tab === "inicio" && (
          <>
            <View style={styles.kpi}>
              <Text style={styles.kpiLabel}>Volume hoje</Text>
            ...
            <View style={styles.grid}>
              <View style={styles.mini}>
                <Text style={styles.miniLabel}>PIX gerados</Text>
                <Text style={styles.miniValue}>{stats?.pixGerados ?? 0}</Text>
              </View>
              <View style={styles.mini}>
                <Text style={styles.miniLabel}>PIX pagos</Text>
                <Text style={styles.miniValue}>{stats?.pixPagos ?? 0}</Text>
              </View>
              <View style={styles.mini}>
                <Text style={styles.miniLabel}>Abandonos</Text>
                <Text style={styles.miniValue}>{stats?.abandonados ?? 0}</Text>
              </View>
              <View style={styles.mini}>
                <Text style={styles.miniLabel}>Conversão</Text>
            ...
            {leads.slice(0, 20).map((l) => (
              <LeadCard key={l.row} lead={l} showPaid />
            ))}
          </>
        )}
        ...
        {tab === "vendas" &&
          (vendas.length === 0 ? (
            <Text style={styles.empty}>Nenhuma venda hoje</Text>
          ) : (
            vendas.map((l) => <LeadCard key={l.row} lead={l} />)
          ))}

        {tab === "carrinhos" &&
          (carrinhos.length === 0 ? (
            <Text style={styles.empty}>Nenhum abandono hoje</Text>
          ) : (
            carrinhos.map((l) => <LeadCard key={l.row} lead={l} />)
          ))}

        {tab === "pix" &&
          (pixList.length === 0 ? (
            <Text style={styles.empty}>Nenhum PIX hoje</Text>
          ) : (
            pixList.map((l) => <LeadCard key={l.row} lead={l} showPaid />)
          ))}
        ...
        {tab === "config" && (
          <View style={styles.card}>
            <View style={styles.switchRow}>
              <View style={{ flex: 1 }}>
                <Text style={styles.cardTitle}>Purchase ao gerar PIX</Text>
                <Text style={styles.cardSub}>Meta Ads</Text>
              </View>
              <Switch
                value={purchaseOnPix}
                onValueChange={async (v) => {
                  setPurchaseOnPix(v);
                  try {
                    await saveConfig(password, { purchaseOnPixGenerate: v });
                  } catch (e: unknown) {
            ...
            <View style={[styles.switchRow, { marginTop: 16 }]}>
              <View style={{ flex: 1 }}>
                <Text style={styles.cardTitle}>Cartão InfinitePay</Text>
                <Text style={styles.cardSub}>Mostrar no checkout</Text>
              </View>
              <Switch
                value={cardEnabled}
                onValueChange={async (v) => {
                  setCardEnabled(v);
                  try {
                    await saveConfig(password, { cardEnabled: v });
                  } catch (e: unknown) {
                    const msg = e instanceof Error ? e.message : "Erro";
                    Alert.alert("Erro", msg);
                  }
...
const styles = StyleSheet.create({
  ...
  headerSub: { fontSize: 12, color: "#0d9488", marginTop: 2 },
  ...
  miniValue: { fontSize: 20, fontWeight: "800", marginTop: 6, color: "#111" },
  section: {
    fontSize: 14,
    fontWeight: "700",
    marginBottom: 8,
  ...
  chipPrimary: { backgroundColor: "#0d9488" },
